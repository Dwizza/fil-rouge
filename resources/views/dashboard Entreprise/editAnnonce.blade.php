...
<div class="container mx-auto max-w-5xl p-6">
    <div class="bg-gradient-to-br from-gray-900 to-gray-800 rounded-2xl shadow-2xl border border-gray-700/50 backdrop-blur-sm overflow-hidden">
        <div class="p-6 border-b border-gray-700/50">
            ...
            <p class="text-gray-400 text-sm mt-1">Update your listing information</p>
        </div>
        ...
        <form action="{{ route('editannonce',$annonce) }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-8">
            @csrf
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input id="title" value="{{$annonce->title}}" name="title" type="text" placeholder="e.g., Modern Loft Apartment" required 
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        ...
                        <input type="number" value="{{$annonce->price}}" name="price" id="price" placeholder="Enter amount" required 
            ...
            <div class="space-y-2">
                ...
                <div class="relative">
                    <textarea id="description" name="description" rows="4" placeholder="Describe the property details" required 
                        class="w-full rounded-lg border border-gray-600 bg-gray-700/50 px-4 py-3 text-gray-200 focus:border-amber-500 focus:ring focus:ring-amber-500/20 placeholder:text-gray-500">{{$annonce->description}}</textarea>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
                ...
                <div class="space-y-2">
                    ...
                    <div class="relative">
                        <input type="text" name="location" value="{{$annonce->location}}" id="location" placeholder="City, State" required 
                            class="w-full rounded-lg border border-gray-600 bg-gray-700/50 px-4 py-3 text-gray-200 focus:border-amber-500 focus:ring focus:ring-amber-500/20 placeholder:text-gray-500">
            ...
            <div class="space-y-2">
                ...
                <!-- Current Photos -->
                <div class="mb-4">
                    <h3 class="text-sm text-gray-400 mb-2">Current Images</h3>
                    
                    @php
                        $images = explode(',', $annonce->image);
                    @endphp

                    <div id="existing-photos" class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4">
                        @foreach ($images as $image)
                            <div class="relative group image-wrapper rounded-lg overflow-hidden border border-gray-700 aspect-square bg-gray-800" data-image="{{ $image }}">
                                <img src="/{{  $image }}" class="w-full h-full object-cover transition-transform group-hover:scale-110 duration-300" alt="Annonce image">
                                <div class="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 group-hover:opacity-100 transition-all flex items-end justify-center pb-2">
                                    <button type="button" class="bg-red-500 text-white rounded-full w-8 h-8 flex items-center justify-center transform transition-transform hover:scale-110"
                ...
                <div id="dropzone" class="mt-4 flex justify-center px-6 pt-5 pb-6 border-2 border-dashed border-gray-500 hover:border-amber-500 rounded-xl bg-gray-800/50 transition-all cursor-pointer group">
